...
- `postgres.test.mjs` proves ordered migrations, advisory locking, checksum drift rejection, PostgreSQL-only authority, atomic first-owner/workspace commitment and bounded readiness failure. `workspace-upgrade.test.mjs` runs only in disposable CI PostgreSQL and proves additive 0001→0002 preservation plus cross-user/workspace read refusal.
- Backend behavior tests prove signed-session login/rejection, encrypted n8n credentials, workspace selection and owner-scoped provider and evidence-list routes, immutable private provider origins, and sanitization before repository calls.
