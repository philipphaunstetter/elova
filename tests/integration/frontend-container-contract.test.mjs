...
const model = yaml.load(readFileSync('ops/container/compose.frontend.yaml', 'utf8')
...
test('VPS frontend is a separate disabled-by-default Traefik project with no host port', () => {
  ...
  assert.deepEqual(frontend.networks, ['elova_frontend_proxy', 'elova_frontend_egress'])
  assert.deepEqual(Object.keys(model.networks).sort(), ['elova_frontend_egress', 'elova_frontend_proxy'])
  assert.equal(model.networks.elova_frontend_proxy.internal, true)
  assert.equal(model.networks.elova_frontend_proxy.external, undefined)
  assert.equal(model.networks.elova_frontend_proxy.name, 'elova_frontend_proxy')
  assert.equal(model.networks.elova_frontend_egress.external, undefined)
  assert.equal(frontend.labels['traefik.enable'], false)
  assert.equal(frontend.labels['traefik.docker.network'], 'elova_frontend_proxy')
