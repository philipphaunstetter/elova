...
The additive `0002_workspaces.sql` migration preserves existing owners as ordinary users, sessions and sanitized provider evidence, backfills a distinct owned workspace and creator membership for each existing owner without granting global access, and scopes n8n connections and evidence to an explicitly selected workspace with member-owner or separately enrolled super-administrator authorization. Authenticated users can create/switch workspaces; public signup, billing, customer enrollment, governance/Jev and scoring remain deferred.
