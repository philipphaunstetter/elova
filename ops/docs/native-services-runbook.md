...
Immediately remove the password handoff file on success or failure and inspect the owner count on uncertain outcomes before retrying. The transaction locks initial enrollment and writes the owner and personal workspace atomically. Existing owners close the bootstrap permanently; migration backfills their personal workspace without recreating them. Never reset/drop the database to repeat bootstrap. No public signup or browser enrollment exists. This runbook does not authorize running the command or accessing production credentials.
