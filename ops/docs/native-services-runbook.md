...
Immediately remove the password handoff file on success or failure and inspect the super-administrator count and the chosen identifier on uncertain outcomes before retrying. The transaction locks initial enrollment and writes the super administrator, personal workspace and creator membership atomically. Existing ordinary owners remain isolated and do not close bootstrap; only an existing super administrator closes it permanently. Migration backfills legacy personal workspaces and creator memberships without recreating or promoting owners. Never reset/drop the database to repeat bootstrap. No public signup or browser enrollment exists. This runbook does not authorize running the command or accessing production credentials.
